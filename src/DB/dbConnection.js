...
Db.prototype.update = async function(collectionName, where, values) {
  if (!this.isConnected()) return Promise.reject();
...
Db.prototype.delete = async function(collectionName, where) {
  if (!this.isConnected()) return Promise.reject();
  ...
  switch (this._dbType.toUpperCase()) {
    case "MONGODB": {
      if (!this.isConnected()) return Promise.reject();
      const dbo = this._db.db(this._dbName);
      const response = await dbo.collection(collectionName).deleteMany(where);
      return response.result;
    }
    default:
      return Promise.reject();
